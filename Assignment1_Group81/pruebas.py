...
def uniform_crossover(pop):
    ...
    for p in range(0, 2*pop.shape[0], 2):
...
for enemy in enemies:  # iterating over the enemies
    ...
    n_hidden_neurons = 20  # hidden neurons per layer
    upper_bound = 1
    lower_bound = -1
    pop_size = 100
    n_generations = 30
    ...
    # External directory
    experiments_dir = 'specialistProbCross_' + str(algorithm) + '_' + str(pop_size) + 'pop_' + str(
        n_generations) + 'gen_enemy' + str(enemy) + '_0' + str(int(mutation_rate * 10)) + 'mut'
    if not os.path.exists(experiments_dir):
        os.makedirs(experiments_dir)

    for r in range(0, repetitions):
        # Creates directory to save experiments per repetition
        experiment_name = experiments_dir + '/specialist_' + str(algorithm) + '_' + 'enemy' + str(enemy) + '_rep' + str(
            r)
        ...
        nn_params = (env.get_num_sensors() + 1) * n_hidden_neurons + (n_hidden_neurons + 1) * n_hidden_neurons + (
                    n_hidden_neurons + 1) * output_size
        ...
        file_aux.write('\n\ngen best mean std')
        print('\n GENERATION ' + str(first_gen) + ' (rep ' + str(r) + ') ' + str(round(fit_pop[best], 6)) + ' ' + str(
            round(mean, 6)) + ' ' + str(round(std, 6)))
        file_aux.write(
            '\n' + str(first_gen) + ' ' + str(round(fit_pop[best], 6)) + ' ' + str(round(mean, 6)) + ' ' + str(
                round(std, 6)))
        file_aux.close()
        ...
        for i in range(first_gen + 1, n_generations):
            ...
            # saves results
            file_aux = open(experiment_name + '/results.txt', 'a')
            print('\n GENERATION ' + str(i) + ' (rep ' + str(r) + ') ' + str(round(fit_pop[best], 6)) + ' ' + str(
                round(mean, 6)) + ' ' + str(round(std, 6)))
